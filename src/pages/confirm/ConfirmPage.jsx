...
function ConfirmPage() {
	...
	useEffect(() => {
		const sendEmail = (data) => {
			axios
				.post('https://api.imanage.services/api/api/nitc', data)
				.then((Response) => {
		...
		const updateTicketCount = async (memberId, userSessions) => {
			try {
				...
				const memberDoc = doc(
					firestore,
					'members',
					memberQuerySnapshot.docs[0].id
				);

				const ticketCount = userSessions.filter(
		...
		const confirmPg = async () => {
			...
			const url =
        'https://e5ncju2y5f.execute-api.eu-west-2.amazonaws.com/prod/confirm';
