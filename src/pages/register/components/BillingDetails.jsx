...
function BillingDetails({
	...
	const handleVerify = async () => {
		...
		if (querySnapshot.docs.length > 0) {
			...
			if (member.ticketCount >= 3) {
				setErrorMsg(
					'You have bought the maximum number of tickets with this member ID.'
				);
	...
	const handleValidateEmail = async (email) => {
		...
		if (querySnapshot.docs.length > 0) {
			...
			const unRegisteredSessions = reg_sessions.filter(
				(session) => session.isRegistered === false
			);
	...
	const handleValidateNic = (nic) => {
		...
		if (
			oldEmailRegex.test(nic) ||
      newEmailRegex.test(nic) ||
      passportRegex.test(nic)
		) {
			setIsNicValid(true);
		} else {
			setIsNicValid(false);
	...
	const handleValidateContanctNo = (contactNo) => {
		...
		if (
			regexContactNo.test(contactNo) ||
      regexContactNoWithPluses.test(contactNo)
		) {
                ...
                Billing details
							...
							<div className="">
								<div className="form-check">
									<input
										...
										id="csslMemberCheck"
									/>
									<label className="form-check-label" htmlFor="csslMemberCheck">
                    I&apos;m a CSSL member.
									</label>
								</div>
                  ...
                  If you are a CSSL member, you will receive a 20% discount.
						...
						{isMember && (
							<div className="col-lg-6 col-sm-12">
								...
								{btnState === 'not-verified' && (
									<p style={{ color: '#f27474', marginTop: '0' }}>{errorMsg}</p>
								)}
							</div>
						)}
					</div>
                          ...
                          This field is required
								...
								<div className="input-validate">
									<input
										...
										style={{
											borderColor: inputError ? '#f27474' : '#ccc !important',
										}}
									/>
                  ...
                  Contact Number
								...
								{isContactNoValid === null ? (
									...
									<span className="input-error">Invalid Contact Number</span>
								)}
								<input
									required
									className="form-control form-control-sm f-input"
