function SheduleDay01() {
	return (
		<section id="shedule_day_01" className="">
			<div className="container">
				<div className="moblie-title">
					<h4>Inaguration</h4>
				</div>

				<div className="row">
					<div className="col-12">
						<div
							className="schedule-area row wow fadeInDown"
							data-wow-delay="0.3s"
						>
							<div className="schedule-tab-content col-12 clearfix">
								<div className="tab-content" id="myTabContent">
									<div className="tab-pane fade show active">
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 6.00 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>
                            Arrival of the Chief Guest & the Guest of Honour
													</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 6.05 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>National Anthem / Lighting the Digital Lamp</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 6.20 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Welcome Address</h4>
												</div>
												<div className="schedule-card-body">
													<span>
                            Address by <strong>Dr. Ajantha Atukorale</strong>,
                            President CSSL
													</span>
													<br />
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 6.30 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>CSSL National ICT Awards 2025 - Session 1</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 6.45 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>
                            Address by the <strong>Chief Guest</strong>
													</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 6.55 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>
                            Address by the <strong>Guest of Honor</strong>
													</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 7.10 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Keynote Address</h4>
												</div>
												<div className="schedule-card-body">
													<span>
                            Address by <strong>Mr. Sandun Hapugoda</strong> ,
                            Country Manager - Sri Lanka and Maldives Mastercards
													</span>
													<br />
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 7.30 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>CSSL National ICT Awards 2025 - Session 2</h4>
												</div>
											</div>
										</div>

										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 7.45 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Keynote Address</h4>
												</div>
												<div className="schedule-card-body">
													<span>
                            Address by <strong>Mr. Lasantha Bogoda</strong>,
                            Director/CEO , DMS Software Technologies (Pvt) Ltd
													</span>
													<br />
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 7.55 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Keynote Address</h4>
												</div>
												<div className="schedule-card-body">
													<span>
                            Address by <strong>Mr. Shanaka de Silva</strong>,
                            Group Finance Director at South Asian Technologies
                            Group
													</span>
													<br />
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 8.05 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Awarding of CSSL Fellow Membership</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 8.15 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Recognition of Sponsors NITC 2025</h4>
												</div>
											</div>
										</div>
										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 8.35 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Vote of Thanks</h4>
												</div>
												<div className="schedule-card-body">
													<span>
                            Address by <strong>Mr. Udesh Seneviratna</strong>,
                            Conference Chair
													</span>
													<br />
												</div>
											</div>
										</div>

										<div className="schedule-card">
											<div className="schedule-slot-time">
												<span> 8.45 PM Onwards</span>
											</div>
											<div className="schedule-card-content">
												<div className="schedule-card-header">
													<h4>Fellowship and Cocktail</h4>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section>
	);
}

export default SheduleDay01;
